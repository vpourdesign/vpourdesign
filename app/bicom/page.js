...
export default function BicomPage() {
  ...
  useEffect(() => {
    fetch('/api/bicom/grille', { method: 'GET' })
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/bicom', {
        method: 'POST',
  ...
  async function logout() {
    await fetch('/api/bicom', { method: 'DELETE' });
    ...
    return (
      <div style={s.loginWrap}>
        <div style={s.loginBox}>
          <div style={s.eyebrow}>V pour Design × BICOM</div>
          <h1 style={s.loginTitle}>
            Grille <em style={{ color: 'var(--accent, #C9A84C)', fontStyle: 'italic' }}>tarifaire</em>
          </h1>
          <p style={s.sub}>Médias sociaux &amp; création de contenu. Document de travail privé.</p>
  ...
  return (
    <div style={s.shell}>
      <div style={s.bar}>
        <span style={s.eyebrow}>V pour Design × BICOM — grille tarifaire</span>
        <span style={s.barActions}>
          <a href="/api/bicom/grille" target="_blank" rel="noopener noreferrer" style={s.link}>
            Plein écran / imprimer
          </a>
          <button onClick={logout} style={s.linkBtn}>Quitter</button>
        </span>
      </div>
      <iframe src="/api/bicom/grille" title="Grille tarifaire BICOM" style={s.frame} />
    </div>
  );
}
